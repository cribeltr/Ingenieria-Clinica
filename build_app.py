...
v0.40 [2026-05-29] Ajustes de la especificación funcional (sin perder funcionalidad).
  - Filtros de columna estilo Excel con selección MÚLTIPLE: en Buscar equipos y Registro MP
    ...
    Helper compartido por ambas planillas (colFiltroActivo/pasaFiltroCol/celdaFiltroCol) para
    no divergir. Los accesos rápidos y la navegación desde el Resumen siguen funcionando.
  - Recepción: el N° de envío se HEREDA del último Envío del equipo (preseleccionado, no a
    mano), igual que el folio. Reconoce la etiqueta histórica 'Envío a Serv. Técnico'.
  - Buscar equipos: se agregó la columna Serie y la vista pasó a llamarse "Buscar equipos".
  - Pendientes: la tabla suma Responsable, Recordatorio y Gestión (último seguimiento); nuevo
    campo Responsable en crear/editar y en el Excel exportado.
  - Estados del pendiente renombrados a la nomenclatura del documento: Creado / Abierto /
    Cerrado (los valores internos no cambian, solo la etiqueta visible).
v0.39 [2026-05-29] Fix: las planillas Equipos y Registro MP quedaban EN BLANCO al cargar.
  - BUG (regresión de v0.37/v0.38): algunos campos del catálogo (modelo en 18 equipos,
    ubicación en 3) vienen como NÚMERO en el origen. Los getters de columna devolvían el
    número crudo (e.modelo||'' -> 840). Al construir el filtro por columna, valoresUnicos
    ordena con localeCompare y un número no tiene ese método -> excepción -> la tabla
    completa NO se renderizaba ("al cargar los datos no se ven"). Además el filtro comparaba
    número vs el texto del <select> (840 !== '840'), por lo que filtrar por ese modelo no
    devolvía nada.
  - Fix: las columnas de texto/lista normalizan SIEMPRE su valor a string (una sola vez por
    vista). Cubre de golpe los 3 sitios del patrón: orden (localeCompare), filtro (val===fv)
    y render de celda. Aplicado en VIEWS.equipos (COLS) y VIEWS.registroMP (cols()).
  - Verificado en jsdom con los datos reales: antes 0 filas / 0 filtros; ahora 893 equipos
    (500 visibles) y los 8 selects de filtro. registroMP y el resto de vistas sin errores.
...
.colf-btn{width:100%;font-size:11px;padding:5px 7px;border-radius:6px;border:1px solid var(--border);background:var(--surface);color:var(--text-2);cursor:pointer;text-align:left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.colf-btn:hover{border-color:var(--accent)}
.colf-btn.activo{border-color:var(--accent);color:var(--accent);font-weight:600}
.colf-pop{position:fixed;z-index:200;min-width:200px;max-width:300px;background:var(--surface);border:1px solid var(--border-strong);border-radius:10px;box-shadow:var(--shadow);padding:8px}
.colf-actions{display:flex;justify-content:space-between;gap:6px;margin-bottom:6px}
.colf-search{width:100%;font-size:12px;padding:6px 8px;border-radius:6px;border:1px solid var(--border);margin-bottom:6px}
.colf-list{max-height:240px;overflow:auto;display:flex;flex-direction:column;gap:1px}
...
// Ciclo de vida del pendiente (nomenclatura de la especificación): Creado -> Abierto -> Cerrado.
// Los valores internos se conservan (no_iniciado/en_proceso/cerrado) para no romper los ~12
// conteos que comparan !== 'cerrado': solo cambia la ETIQUETA visible.
//   no_iniciado = Creado (lo generó el programa, aún sin gestionar)
//   en_proceso  = Abierto (en gestión, con plazo y recordatorio)
//   cerrado     = Cerrado (resuelto, con fecha de cierre)
const ESTADO_PEND_LABEL = {no_iniciado:'Creado', en_proceso:'Abierto', cerrado:'Cerrado'};
...
// ---- Filtro de columna estilo Excel: elegir uno, varios o todos ----
// Compartido por las planillas (Buscar equipos y Registro MP) para no divergir.
// filtros[col.k] guarda un Set de valores elegidos; vacío o ausente = todos.
function colFiltroActivo(filtros, k){ const s = filtros[k]; return s instanceof Set && s.size > 0; }
function pasaFiltroCol(filtros, k, val){ const s = filtros[k]; return !(s instanceof Set) || s.size === 0 || s.has(val); }
// Fija el filtro de una columna a UN valor (accesos rápidos / navegación desde el resumen).
function setFiltroCol(filtros, k, valor){
  if(valor == null || valor === '') delete filtros[k];
  else filtros[k] = new Set([valor]);
}
// ¿Hay algún filtro de columna activo? (Set con elementos o texto en columnas de "contiene".)
function hayFiltroCol(filtros){
  return Object.keys(filtros).some(k => { const v = filtros[k]; return v instanceof Set ? v.size > 0 : !!v; });
}
// Popup único a nivel de body (evita que el contenedor con scroll lo recorte).
function abrirChecklistFiltro(anchorBtn, col, valores, filtros, onChange){
  document.querySelectorAll('.colf-pop').forEach(p => p.remove());
  const cur = filtros[col.k] instanceof Set ? new Set(filtros[col.k]) : new Set();
  const pop = el('div',{class:'colf-pop'});
  const buscar = el('input',{type:'search',class:'colf-search',placeholder:'Buscar valor…'});
  const lista = el('div',{class:'colf-list'});
  const pintar = q => {
    lista.innerHTML = '';
    const qn = (q||'').toLowerCase();
    valores.filter(v => !qn || v.toLowerCase().includes(qn)).forEach(v => {
      const cb = el('input',{type:'checkbox', checked: cur.has(v) ? 'checked' : false,
        onchange: e => { if(e.target.checked) cur.add(v); else cur.delete(v); }});
      lista.appendChild(el('label',{class:'colf-item'}, cb, el('span',{}, v===''?'(vacío)':v)));
    });
  };
  pintar('');
  buscar.addEventListener('input', () => pintar(buscar.value));
  const aplicar = () => { if(cur.size) filtros[col.k] = cur; else delete filtros[col.k]; pop.remove(); onChange(); };
  pop.appendChild(el('div',{class:'colf-actions'},
    el('button',{class:'small ghost',type:'button',onclick:()=>{ delete filtros[col.k]; pop.remove(); onChange(); }},'Todos'),
    el('button',{class:'small primary',type:'button',onclick:aplicar},'Aplicar')
  ));
...
// Celda de filtro (botón) para una columna de lista; abre el checklist.
function celdaFiltroCol(col, valores, filtros, onChange){
  const n = filtros[col.k] instanceof Set ? filtros[col.k].size : 0;
  const btn = el('button',{class:'colf-btn'+(n?' activo':''), type:'button', title:'Filtrar '+col.l,
    onclick: e => { e.stopPropagation(); abrirChecklistFiltro(btn, col, valores, filtros, onChange); }},
    n ? `${n} sel. ▾` : '(todos) ▾');
  return btn;
}
if(typeof window !== 'undefined' && !window.__colfBound){
  window.__colfBound = true;
  document.addEventListener('click', () => document.querySelectorAll('.colf-pop').forEach(p => p.remove()));
}
...
VIEWS.porResolver = function(root){
  ...
  const tarjetaPend = (p, color) => {
    ...
    if(p.estado==='no_iniciado') acts.appendChild(el('button',{class:'small',onclick:()=>cambiarEstadoPend(p,'en_proceso')},'Abrir'));
    acts.appendChild(el('button',{class:'small primary',onclick:()=>cerrarPendiente(p)},'Cerrar'));
  ...
  if(noInic.length){ const s=seccion('Creados', noInic.length+' sin gestionar'); noInic.forEach(p=>s.appendChild(tarjetaPend(p,'gray'))); v.appendChild(s); }
  if(enProc.length){ const s=seccion('Abiertos', enProc.length+' en gestión'); enProc.forEach(p=>s.appendChild(tarjetaPend(p,'amber'))); v.appendChild(s); }
...
VIEWS.equipos = function(root, params){
  ...
  // Algunos campos del catálogo (modelo, ubicación) vienen como número en el origen.
  // Las columnas de texto/lista DEBEN devolver string: el filtro compara con el valor
  // (texto) del <select> y el orden usa localeCompare. Un número crudo rompía ambos
  // y la tabla quedaba EN BLANCO al construir los filtros. Se normaliza una sola vez.
  COLS.forEach(c=>{ if(!c.num){ const _g=c.g; c.g=e=>{ const v=_g(e); return v==null?'':String(v); }; } });
  const filtros = {};
  if(params.estado) setFiltroCol(filtros, 'estado', ESTADO_LABEL[params.estado] || params.estado);
  if(params.servicio) setFiltroCol(filtros, 'servicio', params.servicio);
  ...
  function buildHead(){
    ...
    COLS.forEach(c=>{
      let ctrl;
      if(c.lista){
        // Filtro estilo Excel: elegir uno, varios o todos (checklist en popup).
        ctrl = celdaFiltroCol(c, valoresUnicos(c), filtros, render);
      } else {
  ...
  function render(){
    ...
    let lista = state.equipos.filter(e => {
      ...
      for(const c of COLS){
        const val = c.g(e);
        if(c.lista){ if(!pasaFiltroCol(filtros, c.k, val)) return false; }
        else { const fv = filtros[c.k]; if(fv && !norm(val).includes(norm(fv))) return false; }
      }
  ...
  function renderChips(){
    ...
    const hayCol = hayFiltroCol(filtros);
  ...
  const barraQA = el('div',{class:'quick-access'},
    ...
    _qa('En servicio técnico', state.equipos.filter(e=>e.estado==='en_servicio_tecnico').length, ()=>{limpiarFiltros();setFiltroCol(filtros,'estado','En servicio técnico');render();}, 'st'),
    _qa('No operativos', state.equipos.filter(e=>e.estado==='no_operativo').length, ()=>{limpiarFiltros();setFiltroCol(filtros,'estado','No operativo');render();}, 'noop'),
...
VIEWS.registroMP = function(root){
  ...
  // Las columnas de texto/lista deben entregar string (mismo motivo que en Equipos:
  // modelo/ubicación pueden venir como número y romperían el orden y el filtro).
  const cols = () => (mostrarMeses ? [...IDENT, ...mesCols(), ...FINAL] : [...IDENT, ...FINAL])
    .map(c => c.num ? c : {...c, g:e=>{ const v=c.g(e); return v==null?'':String(v); }});
  ...
  function buildHead(){
    ...
    CS.forEach(c=>{
      let ctrl;
      if(c.lista){ ctrl = celdaFiltroCol(c, valoresUnicos(c), filtros, render); }
  ...
  function render(){
    ...
    let lista = state.equipos.filter(e=>{
      ...
      for(const c of CS){ const val=c.g(e); if(c.lista){ if(!pasaFiltroCol(filtros,c.k,val)) return false; } else { const fv=filtros[c.k]; if(fv && !norm(val).includes(norm(fv))) return false; } }
      return true;
    });
...
VIEWS.pendientes = function(root, params){
  ...
  const selEst = el('select',{},
    ...
    el('option',{value:'no_iniciado',selected:params.estado==='no_iniciado'?'selected':false},'Creado'),
    el('option',{value:'en_proceso',selected:params.estado==='en_proceso'?'selected':false},'Abierto'),
...
function renderPendientesTabla(list){
  ...
  return el('div',{style:{overflow:'auto'}},
    el('table',{class:'data'},
      el('thead',{}, el('tr',{}, ['Equipo','Tipo','Descripción','Responsable','Ejecutor','Compromiso','Recordatorio','Gestión','Estado','Acciones'].map(h=>el('th',{},h)))),
      el('tbody',{}, ...list.map(p => {
        const vencido = p.fechaComp && p.fechaComp < today && p.estado !== 'cerrado';
        const ultSeg = (p.seguimientos && p.seguimientos.length) ? p.seguimientos[p.seguimientos.length-1] : null;
        const gestionTxt = ultSeg ? (fmtFecha(ultSeg.fecha)+': '+ultSeg.texto) : '';
        return el('tr',{},
          el('td',{}, el('strong',{}, p.inv||'—'), el('br'), el('small',{class:'muted'}, p.equipo||'')),
          el('td',{}, el('span',{class:'tag'}, TIPO_PENDIENTE[p.tipo]||p.tipo)),
          el('td',{style:{maxWidth:'320px'}}, p.desc),
          el('td',{}, p.responsable||'—'),
          el('td',{}, p.ejecutor||'—'),
          el('td',{style:vencido?{color:'var(--danger)',fontWeight:'600'}:null}, fmtFecha(p.fechaComp)),
          el('td',{}, fmtFecha(p.proxRecord)),
          el('td',{style:{maxWidth:'260px'},title:gestionTxt}, gestionTxt ? el('small',{}, gestionTxt.length>80?gestionTxt.slice(0,80)+'…':gestionTxt) : el('small',{class:'muted'},'—')),
          el('td',{}, badgePend(p.estado)),
          el('td',{class:'actions'},
            el('button',{class:'small',onclick:()=>abrirPendiente(p)},'Ver'),
            p.estado==='no_iniciado' ? el('button',{class:'small',onclick:()=>cambiarEstadoPend(p,'en_proceso')},'Abrir') : null,
            p.estado !== 'cerrado' ? el('button',{class:'small primary',onclick:()=>cerrarPendiente(p)},'Cerrar') : null,
...
// La Recepción se vincula a su Envío por el N° de envío, HEREDADO (no escrito a mano).
// Igual que el folio: si el equipo tiene Envíos a servicio técnico, se preselecciona el
// más reciente; si no hay ninguno, cae a un campo manual.
function nEnvioRecepcionControl(inv){
  // Reconoce la etiqueta canónica y la variante histórica ('Envío a Serv. Técnico')
  // que traen los respaldos antiguos, para heredar el N° también de esos envíos.
  const esEnvio = t => { const s = (t||'').toLowerCase(); return s.includes('env') && (s.includes('serv') || s.includes('técn') || s.includes('tecn')); };
  const envios = state.eventos
    .filter(ev => ev.inv === inv && !ev.anulado && esEnvio(ev.tipo) && ev.nEnvio)
    .sort((a,b)=>(b.fecha||'').localeCompare(a.fecha||''));
  if(!envios.length) return el('input',{type:'text',placeholder:'N° envío original'});
  const sel = el('select',{},
    ...envios.map(ev => el('option',{value:ev.nEnvio},
      ev.nEnvio + (ev.empresa ? (' · ' + ev.empresa) : '') + (ev.fecha ? (' · ' + fmtFecha(ev.fecha)) : ''))),
    el('option',{value:''},'— otro / a mano —')
  );
  sel.value = envios[0].nEnvio;
  return sel;
}
  ...
  function build(){
    ...
    if(tipo){
      ...
      if(tipo === 'Solicitud de trabajo'){
        ...
        const nEnvio = nEnvioRecepcionControl(invSel);
...
function nuevoPendiente(opts){
  ...
  const resp = el('input',{type:'text',placeholder:'Quién responde por el pendiente (persona o servicio)'});
  ...
  modal({title:'Nuevo pendiente',
    hasUnsavedData: ()=> desc.value.trim() !== '' || resp.value.trim() !== '' || ejec.value !== '' || fComp.value !== '' || fRec.value !== '',
    body: el('div',{},
    el('div',{class:'grid-2'},
    ...
    el('button',{class:'primary',onclick:()=>{
      ...
      const p = {
        ...
        responsable: resp.value.trim()||null,
...
function abrirPendiente(p){
  ...
  const resp = el('input',{type:'text',value:p.responsable||'',placeholder:'Persona o servicio responsable'});
  ...
  modal({title:'Pendiente · '+p.inv, wide:true, body: el('div',{},
    ...
    el('button',{class:'primary',onclick:()=>{
      ...
      p.responsable = resp.value.trim()||null;
...
function exportExcel(){
  ...
  const leeme = [
    ...
    ['Pendientes', 'Todos los pendientes con su estado (Creado / Abierto / Cerrado).'],
  ...
  const pendRows = state.pendientes.filter(p=>!p.anulado).map(p => {
    const ult = (p.seguimientos && p.seguimientos.length) ? p.seguimientos[p.seguimientos.length-1] : null;
    return {
      'ID': p.id, 'N° Inv.': p.inv, 'Equipo': p.equipo||'', 'Servicio': p.servicio||'',
      'Tipo': TIPO_PENDIENTE[p.tipo]||p.tipo, 'Descripción': p.desc||'',
      'Responsable': p.responsable||'', 'Ejecutor': p.ejecutor||'',
      'Fecha creación': fmtFecha(p.fechaCrea), 'Fecha compromiso': fmtFecha(p.fechaComp),
      'Recordatorio': fmtFecha(p.proxRecord), 'Fecha cierre': fmtFecha(p.fechaCierre),
      'Estado': ESTADO_PEND_LABEL[p.estado]||p.estado, 'Origen': p.origen||'',
      'Gestión (última)': ult ? (fmtFecha(ult.fecha)+': '+ult.texto) : ''
    };
  });
  tabla(pendRows, 'Pendientes', [{wch:5},{wch:13},{wch:22},{wch:20},{wch:18},{wch:42},{wch:18},{wch:18},{wch:14},{wch:15},{wch:14},{wch:13},{wch:13},{wch:14},{wch:40}]);
