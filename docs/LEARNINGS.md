...
## [2026-05-29] BUG: las planillas quedaban EN BLANCO al cargar (campos numéricos) (v0.39)

- **Disparador:** el usuario reportó que "la última versión perdió funcionalidades; al
  cargar los datos no se ven". Reproducido cargando el `app.html` real en jsdom con sus
  datos: al navegar a **Equipos**, la tabla quedaba vacía (0 filas, 0 filtros).
- **Confirmado (causa raíz):** algunos campos del catálogo vienen como NÚMERO en el origen
  (verificado en seed.json: `modelo` numérico en 18 equipos, `ubic` en 3). Los getters de
  columna usaban `e.modelo||''`, que devuelve el número crudo (`840`), no string. Al
  construir el filtro por columna, `valoresUnicos(...).sort((a,b)=>a.localeCompare(b,…))`
  llama `localeCompare` sobre un número → `TypeError: a.localeCompare is not a function` →
  excepción dentro de `render()` → la tabla COMPLETA no se dibuja. Mismo patrón en
  `VIEWS.registroMP`. Bug secundario: el filtro comparaba `val !== fv` (número vs texto del
  `<select>`), así que filtrar por ese modelo/ubicación tampoco devolvía nada.
- **Hecho:** las columnas de texto/lista normalizan su valor a string una sola vez por vista
  (envoltorio del getter en `VIEWS.equipos` COLS y en `cols()` de `VIEWS.registroMP`). Un
  solo punto cubre los 3 sitios del patrón: orden (`localeCompare`), filtro (`val===fv`) y
  render de la celda. Las columnas `num` ya devolvían string (`vur`/`ano` incluidas), no se
  tocaron. Versión a 0.39, `app.html` regenerado.
- **Validado:** jsdom con datos reales — antes: 0 filas / 0 selects de filtro; después: 893
  equipos (500 visibles) + 8 filtros; `registroMP`, `porResolver`, `dashboard`, `mp` y
  `conciliacion` navegan sin errores.
- **Heurística:** (1) regresión clásica de este repo — `valoresUnicos`/`localeCompare` se
  añadieron en v0.37/v0.38 asumiendo que TODO getter de texto devuelve string, pero los
  datos reales tienen números donde uno espera texto. Validar SIEMPRE con el catálogo real.
  (2) Cuando un valor alimenta `localeCompare` y se compara con el valor (texto) de un
  control de UI, debe garantizarse como string en el ORIGEN (getter), no parchearlo en cada
  uso. (3) Sigue vigente la regla de oro: el patrón vivía en 2 vistas; arreglar una sin la
  otra habría dejado el bug a medias.
